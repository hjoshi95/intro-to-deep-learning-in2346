...
class RescaleTransform:
    ...
    def __call__(self, images):
        ...
        images = self.min + ((images - self._data_min) * new_range / old_range)
        pass
...
def compute_image_mean_and_std(images):
    ...
    # test yourself.                                                       #
    ########################################################################
    mean = np.mean(images, axis = (0, 1, 2)) #Computes per channel mean
    std = np.std(images, axis = (0, 1, 2)) #Computes per channel standard deviation.
