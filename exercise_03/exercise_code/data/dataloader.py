...
class DataLoader:
    ...
    def __len__(self):
        ...
        """take whole dataset and divide it by batch size to find number of batches"""
        if self.drop_last:
            length = len(self.dataset) // self.batch_size #If we are to drop_last minibatch, round down
        else:
            length = -(-len(self.dataset)//self.batch_size) #If we are to include last minibatch, round up 
        pass
