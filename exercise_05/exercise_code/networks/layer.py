...
def affine_forward(x, w, b):
    ...
    D = np.prod(x.shape[1:]) #Gets products of all the dimensions
    reshape_x = np.reshape(x, (x.shape[0], D)) #reshape x to N x D 
    out = reshape_x.dot(w) + b  # N X D . D X M + M 
...
def affine_backward(dout, cache):
    ...
    reshape_x = np.reshape(x,(x.shape[0], np.prod(x.shape[1:]))) # N X D
    dw = reshape_x.T.dot(dout) #D X N . N x M = D x M 
